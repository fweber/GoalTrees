...
base_path = join(dirname(__file__), "..", "..", "goaltrees")
...
def compare_settings(print_fn=print, ignore_diffs=None):
    ...
    news = [i for i in new_sets if i not in ignore_diffs]
    olds = [i for i in old_sets if i not in ignore_diffs]
    changeds = [i for i in changed_sets if i not in ignore_diffs]
    if news:
        print_fn(f"The following settings are not in your config: {news}")
    if olds:
        print_fn(f"The following settings may be too much in your config: {olds}")
    if changeds:
